...
import { access, readFile } from './fs-promisified';
...
export default function installCertificateAuthority(
    ...
    switch (process.platform) {
        ...
        default:
            throw new Error(
                `Unable to automatically add a root certificate for ${
                    process.platform
                } platform.`
            );
    }
...
async function addToMacTrustStores(
    commonName: string,
    rootCertPath: string
): Promise<void> {
    // Chrome, Safari, system utils
    await exec(
        `sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain -p ssl -p basic "${rootCertPath}"`
    );
    // Firefox
    try {
...
async function addToLinuxTrustStores(
    commonName: string,
    rootCertPath: string
): Promise<void> {
    // system utils
    ...
    await exec(
        `sudo cp ${rootCertPath} /usr/local/share/ca-certificates/${commonName}.crt`
    );
...
async function addCertificateToNSSCertDB(
    commonName: string,
    rootCertPath: string,
    nssDirGlob: string,
    checkForOpenFirefox: boolean
): Promise<void> {
    ...
    if (checkForOpenFirefox) {
        const runningProcesses = await exec('ps aux');
    ...
    await Promise.all(
        dirs.map(async potentialNSSDBDir => {
            try {
                await access(path.join(potentialNSSDBDir, 'cert8.db'));
                await exec(
                    `${certutilPath} -A -d "${potentialNSSDBDir}" -t 'C,,' -i ${rootCertPath} -n ${commonName}`
                );
            } catch (e) {
                try {
                    await access(path.join(potentialNSSDBDir, 'cert9.db'));
                    await exec(
                        `${certutilPath} -A -d "sql:${potentialNSSDBDir}" -t 'C,,' -i ${rootCertPath} -n ${commonName}`
                    );
                } catch (e) {
                    // do nothing
                }
...
async function lookupOrInstallCertutil(): Promise<string | void> {
    if (process.platform === 'darwin' && (await commandExists('brew'))) {
        ...
        try {
            certutilPath = path.join(
                (await exec('brew --prefix nss')).toString().trim(),
                'bin',
                'certutil'
            );
        } catch (e) {
